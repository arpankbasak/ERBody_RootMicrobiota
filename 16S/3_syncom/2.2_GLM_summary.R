...
aa_df$family <- sync.spiked.dat$taxa$phylum[match(aa_df$strain, row.names(sync.spiked.dat$taxa))]
# aa_df$family <- paste(
#   sync.spiked.dat$taxa$phylum[match(aa_df$strain, row.names(sync.spiked.dat$taxa))],
#   sync.spiked.dat$taxa$class[match(aa_df$strain, row.names(sync.spiked.dat$taxa))],
#   sync.spiked.dat$taxa$order[match(aa_df$strain, row.names(sync.spiked.dat$taxa))],
#   sync.spiked.dat$taxa$family[match(aa_df$strain, row.names(sync.spiked.dat$taxa))],
#   sep = "_")
...
aa_df_all$family <- sync.spiked.dat$taxa$class[match(aa_df_all$strain, row.names(sync.spiked.dat$taxa))]
# aa_df_all$family <- paste(
#   sync.spiked.dat$taxa$phylum[match(aa_df_all$strain, row.names(sync.spiked.dat$taxa))],
#   sync.spiked.dat$taxa$class[match(aa_df_all$strain, row.names(sync.spiked.dat$taxa))],
#   sync.spiked.dat$taxa$order[match(aa_df_all$strain, row.names(sync.spiked.dat$taxa))],
#   sync.spiked.dat$taxa$family[match(aa_df_all$strain, row.names(sync.spiked.dat$taxa))],
#   sep = "_")
...
strain_in <- unique(df_prev$strain[df_prev$cut_off == TRUE])

p <- df_prev %>%
ggplot(aes(x = aa)) +
geom_density() +
xlim(0,100) +
geom_vline(xintercept = 10, lty = "dashed", colour = c_red, alpha = 0.6, lwd = 1) +
facet_wrap(dose ~ time, scales = "free_y") +
theme_RTN
# labs(x = "", y = "")

ggsave(p, 
  filename = paste0("./figures/differential_analysis/spiked/density_strains.png"),
...
ggsave(p, 
  filename = paste0("./figures/differential_analysis/spiked/density_strains_all.png"),
           bg="transparent", units = img, 
           width=4, 
...
group_by(family, strain, genotype, dose, time) %>%
summarise(meanAA = mean(AA)) %>%
ungroup() %>%
mutate(strain = factor(strain, levels = de_analysis$strain_clusters),
  # meanAA = ifelse(is.finite(log10(meanAA)), log10(meanAA), NA),
  family = factor(family, levels = taxonomy.default.colours$class[which(.$family %in% taxonomy.default.colours$class)]),
...
geom_raster(alpha=1) +
    ...
    ggsave(fig_1a, filename = paste0("./figures/differential_analysis/spiked/hmap_AA.png"),
...
geom_raster(alpha=1) +
    ...
    ggsave(fig_1a1, filename = paste0("./figures/differential_analysis/spiked/hmap_ra_samplewise.png"),
...
mutate(class = sync.spiked.dat$taxa$class[match(.$strain, row.names(sync.spiked.dat$taxa))]) %>%
mutate(strain = factor(strain, levels = de_analysis$strain_clusters),
  class = factor(class, levels = taxonomy.default.colours$class[which(.$class %in% taxonomy.default.colours$class)]),
  sig = as.factor(PValue < alpha),
  family = as.factor(family),
  logFC = ifelse(abs(logFC) > 1, sign(logFC) * 1, logFC)) %>%
...
geom_tile(fill = NA, width = 0.95, height = 0.95, size = 0.6, aes(colour = sig)) +
    ...
    facet_grid(class ~ dose + time, switch = "both", scales="free", space="free") +
    theme_RTN +
    theme(panel.spacing = unit(0.1, "lines"), panel.border = element_rect(fill="transparent", colour=NA),
          axis.text.x=element_text(angle = 90, size = 6, face = "bold", hjust = 1, vjust = 0.5), 
          strip.text.x = element_text(angle = 90, size = 15, hjust = 0.5, vjust = 0.5),
    ...
    ggsave(fig_1b, filename = paste0("./figures/differential_analysis/spiked/hmap_lfc.png"), 
           bg="transparent", width=7, height=14, limitsize=F, device = "png", dpi = 600)
...
geom_tile(fill = NA, width = 0.95, height = 0.95, size = 0.6, aes(colour = sig)) +
    ...
    theme(panel.spacing = unit(0.1, "lines"), panel.border = element_rect(fill="transparent", colour=NA),
          ...
          legend.text = element_text(size = 10, hjust = 0.5, vjust = 0.5)) +
    labs(y="", x="", fill="log2(FC)vs Col-0 ", colour = ""))
    ggsave(fig_1b1, filename = paste0("./figures/differential_analysis/spiked/hmap_lfc_competency.png"), 
           bg="transparent", width=7, height=14, limitsize=F, device = "png", dpi = 600)
...
data.frame(., row.names = as.character(.$strains))
...
ggsave(strip_comp, filename = paste0("./figures/differential_analysis/spiked/strip_compartments.png"), 
...
theme(axis.text.x=element_text(angle = 90, size = 6, face = "bold", hjust = 1, vjust = 0.5), 
      ...
      legend.text = element_text(size = 10, hjust = 0.5, vjust = 0.5)) +
labs(y="", x="", fill="", colour = ""))
ggsave(strip_taxa, filename = paste0("./figures/differential_analysis/spiked/strip_taxa.png"), 
           bg="transparent", width=3, height=14, limitsize=F, device = "png", dpi = 600)
...
geom_raster(alpha=1) +
scale_fill_manual(values = c(`TRUE` = c_black, `FALSE` = c_white)) +
facet_grid(class ~ ., switch = "both", scales="free", space="free") +
theme_RTN +
theme(axis.text.x=element_text(angle = 90, size = 6, face = "bold", hjust = 1, vjust = 0.5), 
      strip.text.x = element_text(angle = 90, size = 15, hjust = 0.5, vjust = 0.5),
      ...
      legend.text = element_text(size = 10, hjust = 0.5, vjust = 0.5)) +
labs(y="", x="", fill="", colour = ""))
ggsave(strip_prev, filename = paste0("./figures/differential_analysis/spiked/strip_prevelant.png"), 
           bg="transparent", width=3, height=14, limitsize=F, device = "png", dpi = 600)
...
ggsave(sct_plot, "./figures/differential_analysis/spiked/pairwise_integrated_plot.png", 
    units = "in", 
    width = 12, 
    height = 8, 
    bg = "transparent")
...
ggsave(sct_plot_geno "./figures/differential_analysis/spiked/pairwise_integrated_genotype_plot.png", 
    units = "in", 
    width = 7, 
    height = 5, 
    bg = "transparent")
...
ggsave(sct_plot_geno_aa, "./figures/differential_analysis/spiked/pairwise_integrated_aa_genotype_plot.png", 
    units = "in", 
    width = 7, 
    height = 5, 
    bg = "transparent")
...
mclapply(as.character(unique(df_temp$group)),function(x){
  ...
  til <- paste0("Pearson: ", round(stat$estimate, 4), "; p-value: ", c(stat$p.value))
  
  p <- temp %>%
  mutate(
    cyp = ifelse(abs(cyp) >= 1, sign(cyp)*1, cyp), 
    pyk10 = ifelse(abs(cyp) >= 1, sign(pyk10)*1, pyk10),
    direction = as.factor(ifelse(sign(cyp) == sign(pyk10), sign(cyp)*1, 0)),
    text_target = ifelse(sign(cyp) == sign(cyp) && abs(cyp) > 0.5 && abs(pyk10) > 0.5, as.character(strain), "")
    ) %>%
  ggplot(aes(x = (cyp), y = (pyk10))) +
  ...
  geom_point(aes(fill = direction
    ...
    # colour = significance, 
    # alpha = significance
    ), colour = c_black, alpha = 0.7, size = 2, shape = 21) +
  # xlim(c(-6, 6)) +
  # ylim(c(-6, 6)) +
  ...
  #   switch = "both", 
  #   space = "free") +
  scale_fill_manual(values = c(`-1` = c_cudo_magenta, `1` = c_dark_green, `0` = c_black)) +
  # scale_shape_manual(values = c(22,23,24)) +
  ...
  theme_RTN_MDS +
   ...
   labs(x = "LFC_cyp", y = "LFC_pyk10", 
    colour = "", shape = "", fill = "", size = "")
ggsave(plot = p, paste0("./figures/differential_analysis/spiked/pairwise_integrated_genotype_", x,"_plot.png"), 
    units = "in", 
    device = "png",
    width = 4, 
    height = 4, 
...
ggsave(mat_plot, filename = paste0(figs, "/differential_analysis/spiked//hmap_de_summary.png"), 
           bg="transparent", 
           units = "in",
           width=5, height=12, 
...
ggsave(mat_plot, filename = paste0(figs, "/differential_analysis/spiked/hmap_AIO_summary.png"), 
           bg="transparent", 
           units = "in",
           width=25, height=14, 
           limitsize=F, device = "png", dpi = 600)
...
#         ggtern::ggsave(filename = paste0(figs,"/differential_analysis/spiked/ternary_diagram.png"),
