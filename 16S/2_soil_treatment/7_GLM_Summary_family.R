...
# Within genotype across treatment
...
idx <- colnames(temp)[str_detect(colnames(temp), "meanRA$")]
temp.mat_RA <- temp[, c(idx, c("tag", "lineage_family", "taxa_atsphere_strain_lineage"))] %>%
    filter(tag %in% top20) %>%
    # add_column(asvb = row.names(.), .before = 1) %>%
    gather(key = "key", value = "val", convert = FALSE, -tag, -lineage_family, -taxa_atsphere_strain_lineage) %>%
    separate(key, into = c("X", "Genotype", "Treatment", "comparison", "vals"), sep = "_") %>%
...
ggplot(aes(x = Genotype, y = tag, fill = logRA)) +
geom_raster(alpha=1) +
# geom_tile(fill = NA, width = 0.95, height = 0.95, size = 0.6, aes(colour = FDR)) +
scale_fill_gradient2(
        high=c_red, 
...
facet_grid(Treatment ~ Order, switch = "both", scales="free", space="free") +
scale_fill_gradient2(
        high=c_dark_green, 
        mid = c_white,
        low = c_cudo_magenta, 
...
mutate(logRA = log10(meanRA)) %>%
ggplot(aes(y = Genotype, x = tag)) +
geom_point(alpha=0.8, aes(fill = logRA), size = 3, shape = 21, colour = c_black, na.rm = FALSE) +
# geom_tile(alpha=0.5, aes(fill = ra), fill = NA, width = 1, height = 1, size = 0.5) +
facet_grid(Treatment ~ Order, switch = "both", scales="free", space="free") +
scale_fill_gradient2(
        high=c_red, 
...
theme() +
labs(y="", x="", fill="")
ggsave(temp.temp, filename = paste0(figs.out,"/pyk10_vs_cyp_family.png"),
           bg="transparent", 
           width=5, 
           height=5,
           units = img, 
...
scale_fill_gradient2(
    ...
    theme(panel.spacing = unit(0.1, "lines"), 
          ...
          legend.text = element_text(size = 2, hjust = 0.5, vjust = 0.5)) +
    labs(y="", x="", fill="% meanRA")) +
    ggsave(filename = paste0(figs.out,"/RA_asv_heatmap_lfc_sorted_sig_family.png"),
           bg="transparent", 
           width=3, 
           height=8,
           units = img, 
...
scale_colour_manual(values = c(`TRUE` = c_black, `FALSE` = NA)) +
    ...
    theme(panel.spacing = unit(0.1, "lines"), panel.border = element_rect(fill="transparent", colour=NA),
          ...
          legend.text = element_text(size = 2, hjust = 0.5, vjust = 0.5)) +
    labs(y="", x="", fill="")) +
    ggsave(filename = paste0(figs.out,"/SILVA_annotated_lfc_sorted_sig_family.png"),
           bg="transparent", 
           width=2, 
           height=8,
           units = img, 
...
scale_colour_manual(values = c(`FALSE` = NA, `TRUE` = c_black), guide = FALSE) +
    ...
    theme(panel.spacing = unit(0.1, "lines"), 
          ...
          legend.text = element_text(size = 2, hjust = 0.5, vjust = 0.5)) +
    labs(y="", x="", fill="lfc(vCol-0)")) +
    ggsave(filename = paste0(figs.out,"/LFC_asv_heatmap_lfc_km_sorted_sig_family.png"),
           bg="transparent", 
           width=3, 
           height=8,
           units = img, 
...
idx <- colnames(temp)[str_detect(colnames(temp), "genotype_logFC$")]
temp.lfc <- temp[, c("tag", "lineage_family", "taxa_atsphere_strain_lineage", idx)] %>%
    # filter(tag %in% top20) %>%
    # add_column(asvb = row.names(.), .before = 1) %>%
    gather(key = "key", value = "val", convert = FALSE, -tag, -lineage_family, -taxa_atsphere_strain_lineage) %>%
    separate(key, into = c("X", "Genotype", "Treatment", "comparison", "vals"), sep = "_") %>%
    select(-X) %>%
    spread(key = vals, value = val, fill = NA, convert = FALSE) %>%
    spread(key = Genotype, value = logFC, fill = NA, convert = FALSE) %>%
    select(-MS, -PBS) %>%
    mutate(
      group = as.factor(comparison),
      # significant = (abs(logFC) > 0.5 & PValue <= alpha),
...
geom_smooth(method = glm, 
  ...
  aes(group = Treatment, colour = Treatment)
    ...
    # scale_alpha_manual(values = c(`FALSE` = 0.3, `TRUE` = 0.8), guide = FALSE) +
    # scale_size_manual(values = c(0.3, .5, .7, .9, 1.2), guide = FALSE) +
    scale_colour_manual(values = c(`Extract` = c_dark_grey, `Exudate` = c_black), guide = FALSE) +
    scale_fill_manual(values = c(`Extract` = c_dark_grey, `Exudate` = c_black), guide = FALSE) +
    theme_RTN_MDS +
    xlim(c(-2,2)) +
    ylim(c(-2,2)) +
    theme(panel.spacing = unit(0.5, "lines"), 
    ...
    labs(x = "cyp", 
      y = "pyk10")
    ggsave(p, file = paste0(figs.out, "/Parwise_summary_extract_exudate_genotype.png"), 
                dpi = 600, 
                units = img, 
                device = "png", 
                bg = "transparent", 
...
scale_colour_manual(values = c(`FALSE` = NA, `TRUE` = c_black), guide = FALSE) +
    ...
    theme(panel.spacing = unit(0.1, "lines"), 
          ...
          legend.text = element_text(size = 2, hjust = 0.5, vjust = 0.5)) +
    labs(y="", x="", fill="lfc(vs Soil)")) +
    ggsave(filename = paste0(figs.out,"/LFC_asv_heatmap_lfc_km_sorted_vs_soil_sig_family.png"),
           bg="transparent", 
           width=3, 
           height=8,
           units = img, 
...
idt <- taxa.mat$lineage_family[str_detect(taxa.mat$lineage_family, "__Burkhol|__Xantho|__Rhizo|__Baci")]
