...
cor_list <- mclapply(X = c("E", "RP"), FUN = function(x){

  # x = "RP"
  temp <- asv_clustered$lfc_annotated.ra
  # idx <- str_detect(colnames(temp), paste("Col", x, "mean", sep = "_"))
  idy <- str_detect(colnames(temp), paste(x, "cas11", "logFC$|", x, "cas11", "PValue$", sep = "_")) |
  str_detect(colnames(temp), paste(x, "cas13", "logFC$|", x, "cas13", "PValue$", sep = "_"))
  temp <- temp[,(idy)==TRUE] %>% cbind.data.frame(., temp[,c("lineage_family", "taxa_atsphere_strain_lineage")])
  ...
  l0_df <- lapply(X = c(unique(temp$lineage_family)), function(i){


  	# i = sample(as.character(unique(temp$lineage_family)), 1)
  	

  	l1_df <- lapply(X = c("cas11", "cas13"), function(z){

  		l2_df <- lapply(X = c("nai1", "pyk10", "cyp", "myb"), function(m){

  			l3_df <- lapply(X = c("nai1", "pyk10", "cyp", "myb"), function(r){

  				if(m!=r){

  					temp_cor <- temp %>% filter(lineage_family == i) %>%
					  	select(-taxa_atsphere_strain_lineage) %>%
					  	mutate(asvb = row.names(.)) %>%
					  	gather(key = "key", value = "vals", convert = FALSE, -lineage_family, -asvb) %>%
					  	separate(key, into = c("Genotype", "Compartment", "Soil_Batch", "x"), sep = "_") %>%
					  	na.omit(x) %>%
					  	spread(x, value = vals, fill = NA) %>%
					  	mutate(PValue = sum(PValue <= 0.05)) %>%
					  	filter(Soil_Batch == z) %>%
					  	spread(Genotype, value = logFC) %>%
					  	data.frame(., stringsAsFactors = FALSE)

					  	ix <- which(colnames(temp_cor) == m)
					  	iy <- which(colnames(temp_cor) == r)

					  	ct <- cor.test(temp[,ix], temp[,iy])
					  	...
					  	stat_df <- data.frame(
					  		...
					  		genotype_x = m,
					  		genotype_y = r,
					  		n_sig = unique(temp_cor$PValue),
					  		n_asvs = length(unique(temp_cor$asvb)),
					  		PCC = ct$estimate,
					  		PCC_pval = ct$p.value,
  			...
  			stat_df <- do.call(rbind.data.frame, l3_df)
  			return(stat_df)

  	})
  		stat_df <- do.call(rbind.data.frame, l2_df)

  	return(stat_df)

 })

  	stat_df <- do.call(rbind.data.frame, l1_df)
	return(stat_df)
})

	stat_df <- do.call(rbind.data.frame, l0_df)
  ...
  temp <- temp %>%
    ...
    gather(key = "key", value = "val", convert = FALSE, -asvb, -Col, -lineage_asvb, -Atsphere_strain, -cluster) %>%
    ...
    separate(lineage_asvb, into = c("Kingdom", "Phylum", "Class", "Order", "Family", "X2"), sep = ";", remove = TRUE) %>%
    select(-X1, -X2) %>%
    mutate(
...
#   temp <- temp[,idy] %>% cbind.data.frame(., temp[,c("lineage_asvb", "Atsphere_strain", "cluster")])
...
temp <- asv_clusterd$lfc_annotated.ra
...
temp.mat_lfc <- temp[, idx] %>%
    # add_column(asvb = row.names(.), .before = 1) %>%
    gather(key = "key", value = "val", convert = FALSE, -tag, -lineage_family, -taxa_atsphere_strain_lineage) %>%
    ...
    mutate(
      ...
      # cluster = factor(cluster, levels = asv_clusterd$cluster_sorted),
      tag = factor(tag, levels = asv_clusterd$lfc_sorted)) %>%
    # separate(Atsphere_strain, into = c("At_Kingdom", "At_Phylum", "At_Class", "At_Order", "At_Family", "At_strain", "X1"), sep = ";", remove = TRUE) %>%
    separate(lineage_family, into = c("Kingdom", "Phylum", "Class", "Order", "Family"), sep = ";", remove = TRUE) %>%
    mutate(
      Order = str_replace_all(.$Order, "^NA$", "Unclassified")
...
idx <- colnames(temp)[str_detect(colnames(temp), "meanRA$")]
temp.mat_RA <- temp[, c("tag", "lineage_family", "taxa_atsphere_strain_lineage", idx)] %>%
    # add_column(asvb = row.names(.), .before = 1) %>%
    gather(key = "key", value = "val", convert = FALSE, -tag, -lineage_family, -taxa_atsphere_strain_lineage) %>%
    separate(key, into = c("X", "Genotype", "Treatment", "comparison", "vals"), sep = "_") %>%
    select(-X) %>%
...
# ggplot(aes(x = "", y = asvb, fill = At_Family)) +
# geom_raster(alpha=1) +
...
# temp.meta <- rbind.data.frame(meta$datasets[["CAS"]], meta$datasets[["LRO"]], meta$datasets[["ITA"]]) %>%
