...
temp.meta <- data.frame(meta$design_strex[match(temp.meta$SampleID, meta$design_strex$SampleID),]) %>% filter(replicate != 3, Treatment != "Exudate")
...
contrasts.mat <- limma::makeContrasts(
    ...
    strex_pyk10_extract_genotype = pyk10 - Col,
    strex_cyp_extract_genotype = cyp - Col,
    strex_PBS_extract_genotype = PBS - Col,
    
    strex_Col_extract_soil = Col - Soil,
    strex_pyk10_extract_soil = pyk10 - Soil,
    strex_cyp_extract_soil = cyp - Soil,
    strex_PBS_extract_soil = PBS - Soil,
...
write.table(logFC_P, paste0(stats.out, "/ST_stats_logFC.P_combat_Extract_family.txt"), sep="\t", 
...
write.table(count.mat, paste0(stats.out, "/ST_stats_number_of_enriched_depleted_combat_Extract_family.txt"), 
...
write.table(DE, paste0(stats.out, "/ST_stats_enrichment_test.significance_table_combat_Extract_family.txt"), sep="\t", 
            quote = T, row.names = T, col.names = NA)

log2cpm <- cpm(de, prior.count = 2, log = T)

write.table(log2cpm, paste0(stats.out, "/ST_stats_log2cpm_combat_Extract_family.txt"), sep="\t", 
            quote = F, col.names = NA, row.names = T)


# Make object an RObject as output
ST_DE_family_Extract <- list(
  logFC_P = logFC_P, 
  DE_table = count.mat,
  cpm = log2cpm
)

save(list = "ST_DE_family_Extract", file = paste0("./data/ST_DE_asv_Extract_family.RData"))
