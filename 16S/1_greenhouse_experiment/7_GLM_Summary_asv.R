...
lfc_mat <- asv_clustered$lfc_annotated.ra
mean_df <- 100*rowSums(lfc_mat[, str_detect(colnames(lfc_mat), "^Col")])
imc <- (colnames(lfc_mat))[str_detect(colnames(lfc_mat), "^genotype") & str_detect(colnames(lfc_mat), "logFC$")]



# Pairwise scatterplot plot -- store plot objects
mclapply(X = imc, FUN = function(x){
	...
	for(i in imc){
	 ...
	 if(i != x){
	 	...
	 	id_x <- str_detect(colnames(temp), x)
	 	id_y <- str_detect(colnames(temp), i)

		temp.geno <- cbind.data.frame(X_lfc = temp[, id_x], Y_lfc = temp[, id_y], tag = row.names(temp))
		# colnames(temp.geno)[id_x] <- "X_lfc"
		# colnames(temp.geno)[id_y] <- "Y_lfc"



	        stat <- cor.test(temp.geno$X_lfc, temp.geno$Y_lfc) 
	        til <- paste0("Cor: ", round(stat$estimate, 4),"; t: ", round(stat$statistic, 4), "; p: ", stat$p.value)

	        fi <- str_replace_all(paste0(x, "_", i), "genotype_", "")
	        fi <- str_replace_all(fi, "_logFC", "")

	        # SLIVA  
	        temp.geno %>%
	        ggplot(aes(x = X_lfc, y = Y_lfc)) +
	        ggtitle(paste0(fi, "_", til)) +
	        ...
	        geom_point(alpha = 0.6, colour = c_grey, fill = c_black, shape = 22) +
	        # geom_point(aes(
	        #       alpha = alp, 
	        #       size = siz,
	        #       colour = FDR,
	        ...
	        labs(x = x, 
	          y = i) +
	        ggsave(file = paste0(figs.out, "/Parwise_", fi,"_logFC.png"), 
	                    dpi = 600, 
	                    units = img, 
	                    device = "png", 
	                    bg = "transparent", 
...
# add_column(tag = taxa.mat[match(row.names(.), taxa.mat$asvb), "lineage_family"], .before = 1) %>%
...
cbind.data.frame(., taxa.mat[match(.$asv, taxa.mat$asvb), c("Phylum", "Class", "Order")]) %>%
