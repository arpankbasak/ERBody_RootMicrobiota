...
# Make factors
temp.meta <- temp.meta %>%
# filter(!Compartment %in% c("rhizosphere", "rhizoplane"), soil == "CAS") %>%
...
write.table(logFC_P, paste0(stats.out, "/stats_logFC.P_family.txt"), sep="\t", 
...
write.table(count.mat, paste0(stats.out, "/stats_number_of_enriched_depleted_family.txt"), 
...
write.table(DE, paste0(stats.out, "/stats_enrichment_test.significance_table_family.txt"), sep="\t", 
            quote = T, row.names = T, col.names = NA)

log2cpm <- cpm(de, prior.count = 2, log = T)

write.table(log2cpm, paste0(stats.out, "/stats_log2cpm_family.txt"), sep="\t", 
            quote = F, col.names = NA, row.names = T)
...
save(list = "CAS_DE_family", file = paste0("./data/CAS_DE_family.RData"))
