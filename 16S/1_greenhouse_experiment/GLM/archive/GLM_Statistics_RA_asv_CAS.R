...
figs.out <- paste0(figs, paste0("/gh/differential_analysis/uspiked/ra_asv/", pro))
stats.out <- paste0(stats, paste0("/gh/differential_analysis/uspiked/ra_asv/", pro))
out.out <- paste0(out, paste0("/gh/differential_analysis/uspiked/ra_asv/", pro))
...
# asv <- apply(temp.mat[idx,], 2, function(x) x/sum(x))
asv <- temp.mat[idx,]
...
delist <- calcNormFactors(delist, method = "TMMwsp")
...
save(list = "CAS_DE_asv_E", file = paste0("./data/ra_asvs_CAS_DE.RData"))
