...
# Make factors
temp.meta <- temp.meta %>%
filter(!Compartment %in% c("rhizosphere", "rhizoplane"), Soil_Batch == "CAS11") %>%
mutate(
...
contrasts.mat <- limma::makeContrasts(
    ...
    compartment_Col_E_cas11 = (Col_E - soil_BS),
    # compartment_Col_RP = (Col_RP - soil_BS),

    compartment_nai1_E_cas11 = (nai1_E - soil_BS),
    # compartment_nai1_RP = (nai1_RP - soil_BS),

    compartment_pyk10_E_cas11 = (pyk10_E - soil_BS),
    # compartment_pyk10_RP = (pyk10_RP - soil_BS),

    compartment_cyp_E_cas11 = (cyp_E - soil_BS),
    # compartment_cyp_RP = (cyp_RP - soil_BS),

    compartment_myb_E_cas11 = (myb_E - soil_BS),
    # compartment_myb_RP = (myb_RP - soil_BS),

    genotype_nai1_E_cas11          = (  nai1_E   - Col_E ),
    #nai2_E          = (  nai2_E   - Col_E ),
    genotype_pyk10_E_cas11         = ( pyk10_E   - Col_E ),
    genotype_cyp_E_cas11           = (   cyp_E   - Col_E ),
    genotype_myb_E_cas11           = (   myb_E   - Col_E ),
    # bhlh_E = (   bhlh_E   - Col_E ),
...
write.table(logFC_P, paste0(stats.out, "/CAS11_stats_logFC.P_combat_E.txt"), sep="\t", 
...
write.table(count.mat, paste0(stats.out, "/CAS11_stats_number_of_enriched_depleted_combat_E.txt"), 
...
write.table(DE, paste0(stats.out, "/CAS11_stats_enrichment_test.significance_table_combat_E.txt"), sep="\t", 
            quote = T, row.names = T, col.names = NA)

log2cpm <- cpm(de, prior.count = 2, log = T)

write.table(log2cpm, paste0(stats.out, "/CAS11_stats_log2cpm_combat_E.txt"), sep="\t", 
            quote = F, col.names = NA, row.names = T)
...
save(list = "CAS11_DE_asv_E", file = paste0("./data/CAS11_DE_asv_E.RData"))
