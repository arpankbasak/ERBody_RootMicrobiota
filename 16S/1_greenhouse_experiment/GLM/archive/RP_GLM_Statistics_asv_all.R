...
contrasts.mat <- limma::makeContrasts(
    ...
    compartment_Col_RP_gh = (Col_RP - soil_BS),

    # compartment_nai1_E = (nai1_E - soil_BS),
    compartment_nai1_RP_gh = (nai1_RP - soil_BS),

    # compartment_pyk10_E = (pyk10_E - soil_BS),
    compartment_pyk10_RP_gh = (pyk10_RP - soil_BS),

    # compartment_cyp_E = (cyp_E - soil_BS),
    compartment_cyp_RP_gh = (cyp_RP - soil_BS),

    # compartment_myb_E = (myb_E - soil_BS),
    compartment_myb_RP_gh = (myb_RP - soil_BS),
    ...
    genotype_nai1_RP_gh     = (  nai1_RP -   Col_RP),
    # nai2_RP     = (  nai2_RP -   Col_RP),
    genotype_pyk10_RP_gh    = ( pyk10_RP -   Col_RP),
    genotype_cyp_RP_gh      = (   cyp_RP -   Col_RP  ),
    genotype_myb_RP_gh      = (   myb_RP -    Col_RP),
...
write.table(logFC_P, paste0(stats.out, "/GH_stats_logFC.P_combat_RP.txt"), sep="\t", 
...
write.table(count.mat, paste0(stats.out, "/GH_stats_number_of_enriched_depleted_combat_RP.txt"), 
...
write.table(DE, paste0(stats.out, "/GH_stats_enrichment_test.significance_table_combat_RP.txt"), sep="\t", 
            quote = T, row.names = T, col.names = NA)

log2cpm <- cpm(de, prior.count = 2, log = T)

write.table(log2cpm, paste0(stats.out, "/GH_stats_log2cpm_combat_RP.txt"), sep="\t", 
            quote = F, col.names = NA, row.names = T)
...
save(list = "GH_DE_asv_RP", file = paste0("./data/asvs_GH_DE_asv_RP.RData"))
