...
cluster_taxa_reps <- family_clustered$lfc_annotated.ra %>%
...
lfc_mat <- family_clustered$lfc_annotated.ra
...
mclapply(X = imc, FUN = function(x){
	...
	for(i in imc){
	 ...
	 if(i != x){
	 	...
	 	temp <- family_clustered$lfc_annotated.ra
	 	id_x <- str_detect(colnames(temp), x)
	 	id_y <- str_detect(colnames(temp), i)

		temp.geno <- cbind.data.frame(X_lfc = temp[, id_x], Y_lfc = temp[, id_y], tag = temp$lineage_family)
...
temp.mat_lfc <- temp[, idx] %>%
    ...
    mutate(
      ...
      # cluster = factor(cluster, levels = family_clustered$cluster_sorted),
      tag = factor(tag, levels = family_clustered$lfc_sorted)) %>%
    # separate(Atsphere_strain, into = c("At_Kingdom", "At_Phylum", "At_Class", "At_Order", "At_Family", "At_strain", "X1"), sep = ";", remove = TRUE) %>%
    separate(lineage_family, into = c("Kingdom", "Phylum", "Class", "Order", "Family"), sep = ";", remove = TRUE) %>%
    mutate(
      Order = str_replace_all(.$Order, "^NA$", "Unclassified")
...
geom_raster(alpha=1) +
    ...
    theme(panel.spacing = unit(0.1, "lines"), 
          ...
          legend.text = element_text(size = 2, angle = 180, hjust = 0.5, vjust = 0.5)) +
...
geom_raster(alpha=1) +
geom_tile(fill = NA, width = 0.95, height = 0.95, size = 0.3, aes(colour = FDR)) +
scale_fill_gradient2(
        high=c_dark_green, 
        mid = c_white,
        low = c_cudo_magenta, 
