...
data.frame(., row.names = .$given_id) %>% select(group)
...
mat <- mat[,row.names(design)]

# Build contrast
contrast.mat <- limma::makeContrasts(
# Effect of genotype
  Col = (Col_Cologne_A - Col_Krakow),
  # nai1_Krakow = (nai1_Krakow - Col_Krakow),
  pyk10 = (pyk10_Cologne_A - pyk10_Krakow),
  cyp = (cyp_Cologne_A - cyp_Krakow),
  # myb_Krakow = (myb_Krakow - Col_Krakow),
...
table$contrast <- str_replace_all(contrast.names[x], "_A", "")
...
# Variations
lfc_sig <- logFC_P.long %>% filter(PValue <= alpha)

set_lfc <- list(
  Col = lfc_sig %>% filter(contrast == "Col") %>% select(peak_id) %>% unlist(., use.names = FALSE), 
  pyk10 = lfc_sig %>% filter(contrast == "pyk10") %>% select(peak_id) %>% unlist(., use.names = FALSE), 
  cyp = lfc_sig %>% filter(contrast == "cyp") %>% select(peak_id) %>% unlist(., use.names = FALSE)
  )

VennDiagram::venn.diagram(
  x = set_lfc,
  category.names = names(set_lfc),
  filename = paste0(figs, "vennd_experiment.png"),
  imagetype="png" ,
  height = 480 , 
  width = 480 , 
  resolution = 300,
  cex = 0.5,
  cat.cex = 0.5,
  fill = genotype$colours[match(names(set_lfc), genotype$short)],
  cat.fontfamily = "sans",
  cat.pos = c(-27, 27, 135),
  cat.dist = c(0.055, 0.055, 0.085),
  cat.default.pos = "outer",
  cat.col = c("white", "white", "white"),
  output=TRUE
)
...
colnames(table) <- paste(str_replace_all(contrast.names[x], "_A", ""), colnames(table), sep="_")
return(table)
})
...
paste(stats, "/logFC_P_long_annotated_experiment.txt", sep = ""), 
sep = "\t")

write.table(logFC_P.wide, 
paste(stats, "/logFC_P_wide_annotated__experiment.txt", sep = ""), 
sep = "\t")
...
save(list = "lfc_obj", file = paste("./data/lfc_DAMs_experiment.Rdata", sep = ""))
