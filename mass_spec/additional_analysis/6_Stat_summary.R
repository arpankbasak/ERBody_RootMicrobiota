...
load(paste0("./data/lfc_DAMs.Rdata"))
...
# mclapply(c(names(DAM_glm)), function(x){
  ...
  separate(col = contrast, into = c("Genotype", "batch"), sep = "_") %>%
  mutate(
    Genotype = factor(Genotype, levels = geno$short[which(geno$short %in% .$Genotype)]),
  ...
  labs(x = "", y = "", fill = "logFC")
  ggsave(p, file = paste0(figs, "heatmap_lfc.png"),
  units = "in",
  device = "png",
  width = 4.5,
  height = 7.5
  ...
  theme(axis.text.y = element_blank()) +
  labs(x = "", y = "", fill = "logFC")
  ggsave(p, file = paste0(figs, "heatmap_lfc_sig.png"),
  units = "in",
  device = "png",
  width = 4.5,
  height = 4.5
  ...
  #                                 ) +
  # ggsave(file = paste0("./figures/Parwise_metabolome_cyp_pyk_logFC.png"), 
  #                   dpi = 600, 
  #                   units = img, 
  #                   device = "png", 
  #                   width = 4, 
  ...
  enriched <- list(
      nai1 = row.names(df)[
          which(sign(df[,str_detect(colnames(df), "nai1_Krakow_logFC")]) == 1 )],
      pyk10 = row.names(df)[
          which(sign(df[,str_detect(colnames(df), "pyk10_Krakow_logFC")]) == 1)],
      cyp = row.names(df)[
          which(sign(df[,str_detect(colnames(df), "cyp_Krakow_logFC")]) == 1)],
      myb = row.names(df)[
      which(sign(df[,str_detect(colnames(df), "myb_Krakow_logFC")]) == 1)]
  ...
  VennDiagram::venn.diagram(x = enriched,
                            ...
                            filename = paste(figs, "/DAMs-enrichment_VenD_Krakow.png", sep = ""),
                            imagetype = "png",
                            sigdigs = 2,
                            hyper.test = TRUE, 
                            lower.tail = TRUE,
  ...
  VennDiagram::venn.diagram(x = depleted,
                            ...
                            filename = paste(figs, "/DAMs-depleted_VenD_Krakow.png", sep = ""),
                            imagetype = "png",
                            sigdigs = 2,
                            hyper.test = TRUE, 
                            lower.tail = TRUE,
  ...
  enriched <- list(
      # nai1 = row.names(df)[
          # which(sign(df[,str_detect(colnames(df), "nai1_Cologne_logFC")]) == 1 )],
      pyk10 = row.names(df)[
          which(sign(df[,str_detect(colnames(df), "pyk10_Cologne_logFC")]) == 1)],
      cyp = row.names(df)[
          which(sign(df[,str_detect(colnames(df), "cyp_Cologne_logFC")]) == 1)]
      # myb = row.names(df)[
      # which(sign(df[,str_detect(colnames(df), "myb_Cologne_logFC")]) == 1)]
  ...
  VennDiagram::venn.diagram(x = enriched,
                            category.names = names(enriched),
                            filename = paste(figs, "/DAMs-enrichment_VenD_Cologne.png", sep = ""),
                            imagetype = "png",
                            sigdigs = 2,
                            hyper.test = TRUE, 
                            lower.tail = TRUE,
  ...
  VennDiagram::venn.diagram(x = depleted,
                            category.names = names(depleted),
                            filename = paste(figs, "/DAMs-depleted_VenD_Cologne.png", sep = ""),
                            imagetype = "png",
                            sigdigs = 2,
                            hyper.test = TRUE, 
                            lower.tail = TRUE,
...
mutate(
  # MZ = factor(MZ, levels = lfc_sorted),
  log_ra = ifelse(!is.finite(log10(RA)), NA, log10(RA))
  ) %>%
data.frame(.)

# plot(, pch=20 , cex=1.5 , col="#69b3a2")
# ggpairs(as.data.frame(log10(mat)+1e-9)) %>% 
# ggsave(filename = paste(figs, "/correlation_between_samples.png", sep = ""), 
#   device = "png",
#   width = 30, 
#   height = 30, 
#   bg = c_white, 
...
parallel::mclapply(target, function(x){
  ...
  # separate(mz, into = c("MZ", "RT", "mode"), sep = ".\\/", remove = FALSE) %>%
  mutate(
    MZ = round(as.numeric(MZ), 2),
    fil = match(MZ, as.numeric(x))
    ) %>%
  filter(fil ==TRUE)
...
mutate(log_ra = (ifelse(!is.finite(log10(mean_ra)), NA, log10(mean_ra))),
  ...
  labs(x = "", y = "", fill = "log10RA")
  ggsave(p, file = paste0(figs, "heatmap_meanra.png"),
  units = "in",
  device = "png",
  width = 4.5,
  height = 7.5
...
mutate(log_ra = (ifelse(!is.finite(log10(RA)), NA, log10(RA))),
  ...
  labs(x = "", y = "", fill = "log10RA")
  ggsave(p, file = paste0(figs, "heatmap_ra_daignosis.png"),
  units = "in",
  device = "png",
  width = 7.5,
  height = 4.5
...
mutate(mz = factor(mz, levels = lfc_sorted)) %>%
group_by(mz, genotype, batch) %>%
summarise(mean_ra = mean(RA)) %>%
mutate(log_ra = (ifelse(!is.finite(log10(mean_ra)), NA, log10(mean_ra))),
  scale_ra = scale(mean_ra)) %>%
  ...
  theme(axis.text.y = element_blank()) +
  labs(x = "", y = "", fill = "log10RA")
  ggsave(p, file = paste0(figs, "heatmap_meanra_sig.png"),
  units = "in",
  device = "png",
  width = 2.5,
  height = 4.5
...
mutate(log_ra = (ifelse(!is.finite(log10(RA)), NA, log10(RA))),
  ...
  ggsave(p, file = paste0(figs, "boxplot_ra_daignosis.png"),
  units = "in",
  device = "png",
  width = 6,
  height = 7
