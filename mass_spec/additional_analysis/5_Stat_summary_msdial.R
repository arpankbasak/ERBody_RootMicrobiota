...
df_target <- target_msd_mar %>% add_column(ID = row.names(.)) %>%
gather(key = "SampleID", value = "intensity", convert = FALSE, -ID) %>%
cbind.data.frame(., target_msd[match(.$ID, row.names(target_msd)),])

fdata <- (ms_data$fdata[1:32])
nitrile_group <-  grep(unique(fdata$SMILES), pattern = "C#N", ignore.case=TRUE, value = TRUE)
thiol_group <-  grep(unique(fdata$SMILES), pattern = "[N-]=C=S", ignore.case=TRUE, value = TRUE)
isothio_group <-  grep(unique(fdata$SMILES), pattern = "N=C=S", ignore.case=TRUE, value = TRUE)
indole_group <-  grep(unique(fdata$`Metabolite.name`), pattern = "indole|carbinol|camalexin|brassinin|glucosinolate|nitrile|auxin|scopoletin|esculetin", ignore.case=TRUE, value = TRUE)

# grep(unique(fdata$`Metabolite.name`), pattern = "camalexin", ignore.case=TRUE, value = TRUE)
# fdata[fdata$`Metabolite.name` == "w/o MS2:Camalexin",]


# Another set
target <- df_target %>%
mutate(intensity = log2(as.numeric(intensity)+1)) %>%
cbind.data.frame(., meta[match(.$SampleID, meta$ID), c("batch", "genotype")])

target_raw <- df_target %>%
mutate(intensity = as.numeric(intensity)) %>%
cbind.data.frame(., meta[match(.$SampleID, meta$ID), c("batch", "genotype")])

target_col <- target %>% group_by(metabolome_name, batch, genotype) %>%
summarise(x = mean(intensity)) %>%
spread(key = genotype, value = x, fill = 0) %>%
mutate(id = paste(metabolome_name, batch, sep = "_")) %>%
data.frame(., stringsAsFactors = FALSE)

target_norm <- target %>%
mutate(id = paste(metabolome_name, batch, sep = "_")) %>%
mutate(intensity_col = (intensity/target_col$Col[match(.$id, target_col$id)]),
...
target_norm$batch_cor <- df_target_col$batch_cor[match(target_norm$metabolome_name, df_target_col$metabolome_name)]
...
ggplot(aes(x = genotype, y = 2^(log2(intensity+1)))) +
geom_point(
  size = 0.5, 
  # shape = 21, 
  ...
  colour = c_black) +
    ...
    facet_wrap(.~ metabolome_name, scale = "free", switch = "x") +
    ...
    labs(x = "", y = "log2(Peak area)", fill = "")

ggsave(p, file = paste0(figs, "boxplot_targets_all_using_raw_peak.png"),
    units = "in",
    device = "png",
    width = 5,
    height = 4.5,
...
geom_point(
  ...
  colour = c_black) +
geom_boxplot(outlier.alpha = 0, aes(colour = genotype), fill = c_white, alpha = 0.6) +
geom_point(data = target_norm %>% filter(genotype == "Col"), aes(x = genotype, y = ((Col)/10), shape = batch), 
  fill = c_white, 
  ...
  override.aes = TRUE) +
    ...
    theme(
      ...
      axis.text.x = element_blank(),
      strip.text.x = element_text(size = 1)) +
    labs(x = "", y = "Relative abundance normalised to Col-0", fill = "")

ggsave(p, file = paste0(figs, "boxplot_targets_all_using_raw_peak_norm_col.png"),
    units = "in",
    device = "png",
    width = 5,
    height = 4.5,
...
ggsave(p, file = paste0(figs, "boxplot_targets_all_using_raw_peak_norm_col_batch_mean.png"),
    units = "in",
    device = "png",
    width = 5,
    height = 4.5,
...
# The previous set
target <- target_bar[, c("mzl", "rt", "metabolite_name", "class", meta$ID)] %>%
gather(key = "sample", value = "intensity", convert = FALSE, -mzl,-rt, -metabolite_name, -class) %>%
mutate(intensity = 2^as.numeric(intensity)) %>%
cbind.data.frame(., meta[match(.$sample, meta$ID), c("batch", "genotype")])

target_raw <- target_bar[, c("mzl", "rt", "metabolite_name", "class", meta$ID)] %>%
gather(key = "sample", value = "intensity", convert = FALSE, -mzl,-rt, -metabolite_name, -class) %>%
mutate(intensity = as.numeric(intensity)) %>%
cbind.data.frame(., meta[match(.$sample, meta$ID), c("batch", "genotype")])
...
stat_list <- mclapply(unique(target_norm$metabolite_name), function(x){
  ...
  fit <- aov(lm(intensity_col ~ genotype + batch, data = target_norm %>% filter(metabolite_name == x)))
...
ggsave(p, file = paste0(figs, "boxplot_targets_all_batch_norm.png"),
    units = "in",
    device = "png",
    width = 5,
    height = 4.5,
...
geom_point(
  ...
  colour = c_black) +
geom_boxplot(outlier.alpha = 0, aes(colour = genotype), fill = c_white, alpha = 0.6) +
geom_point(data = target_norm %>% filter(genotype == "Col"), aes(x = genotype, y = (Col+1), shape = batch), 
  fill = c_white, 
  colour = c_black, alpha = 0.6,
  size = 1, 
  override.aes = TRUE) +
...
ggsave(p, file = paste0(figs, "boxplot_targets_all_col_marked.png"),
    units = "in",
    device = "png",
    width = 5,
    height = 4.5,
...
geom_point(
  ...
  colour = c_black) +
    ...
    theme(
      ...
      axis.text.x = element_blank(),
      strip.text.x = element_text(size = 1)) +
    labs(x = "", y = "log2(Peak intensity)", fill = "")

ggsave(p, file = paste0(figs, "boxplot_targets_all_raw.png"),
    units = "in",
    device = "png",
    width = 5,
    height = 4.5,
...
# Boxplots for indolic comounds and camalexin
idx <- row.names(fdata)[fdata$`Metabolite.name` %in% indole_group]

# Subset
mat_sub <- as.data.frame(mat[idx,]) %>%
add_column(al_id = row.names(.)) %>%
gather(key = "ID", value = "intensity", convert = FALSE, -al_id)

mat_sub <- cbind.data.frame(
  mat_sub, 
  meta[match(mat_sub$ID, meta$ID), c("genotype", "batch", "group", "replicate")]
)

mat_sub$metabolite <- fdata$`Metabolite.name`[match(mat_sub$al_id, row.names(fdata))]

p <- mat_sub %>% 
# filter(batch == "Cologne_A") %>%
ggplot(aes(x = genotype, y = log2(intensity+1))) +
geom_point(
  size = 0.5, 
  # shape = 21, 
  ...
  colour = c_black) +
    geom_boxplot(outlier.alpha = 0, aes(colour = genotype), fill = c_white, alpha = 0.6) +
    scale_fill_manual(values= genotype$colours[which(genotype$short %in% mat_sub$genotype)]) +
    facet_wrap(.~ metabolite, scale = "free", switch = "x") +
    scale_colour_manual(values= genotype$colours[which(genotype$short %in% mat_sub$genotype)]) +
    scale_shape_manual(values = c(21, 23)) +
    theme_RTN +
    theme(
      axis.text.y = element_text(size = 6), 
      ...
      strip.text.x = element_text(size = 1)) +
    labs(x = "", y = "log2(intensity)", fill = "", colour = "", shape = "")

ggsave(p, file = paste0(figs, "boxplot_ra_all.png"),
    units = "in",
    device = "png",
    width = 6,
    height = 7,
...
stat_list <- mclapply(unique(mat_sub$metabolite), function(x){

  fit <- aov(lm(intensity/1000 ~ genotype:batch, data = mat_sub %>% filter(metabolite == x)))
  stat <- broom::tidy(TukeyHSD(fit, method = "bonferroni")) %>%
  separate(contrast, into = c("A", "B"), sep = "-") %>%
  separate(A, into = c("Genotype_A", "batch_A"), sep = ":") %>%
  separate(B, into = c("Genotype_B", "batch_B"), sep = ":") %>%
  filter(batch_A == batch_B, Genotype_B == "Col") %>%
  mutate(metabolite_name = x) %>%
  data.frame(., stringsAsFactors = FALSE)

  return(stat)
}, mc.cores = 2) %>% do.call(rbind.data.frame, .) %>% arrange(desc(adj.p.value)) %>%
mutate(significant = ifelse(adj.p.value <= 0.05, "*", ""))

write.table(stat_list, "./statistics/indole_metabolom.txt", sep = "\t", quote = FALSE, row.names = FALSE)


# Nitrile
idx <- row.names(fdata)[fdata$`SMILES` %in% c(nitrile_group, thiol_group)]
...
p <- mat_sub %>% 
group_by(metabolite, batch, genotype, ID) %>%
ggplot(aes(x = genotype, y = log2(intensity+1))) +
geom_point(
  size = 0.5, 
  # shape = 21, 
  ...
  colour = c_black) +
    ...
    theme(
      ...
      axis.text.x = element_text(angle = 90, size = 6),
      strip.text.x = element_text(size = 1)) +
    labs(x = "", y = "log2(intensity+1)", fill = "")

ggsave(p, file = paste0(figs, "boxplot_ra_nitriles_thiols_all.png"),
    units = "in",
    device = "png",
    width = 5,
    height = 5,
...
# Enrichment
mat_sub$SMILES <- fdata$`SMILES`[match(mat_sub$al_id, row.names(fdata))]
mat_stat <- mat_sub %>%
group_by(metabolite, SMILES, batch, genotype) %>%
summarise(tot_int = mean(intensity)) %>%
mutate(group = ifelse(SMILES %in% nitrile_group, "Nitriles", ifelse(SMILES %in% thiol_group, "Thiols", "Unkown SMILES"))) %>%
data.frame(.)

mat_smiles <- 
 mat_sub %>%
group_by(SMILES, ID) %>%
summarise(tot_int = sum(intensity)) %>%
spread(key = ID, value = tot_int) %>%
data.frame(., row.names = .$SMILES)

mat_smiles <- apply(mat_smiles[,-1], 2, function(x) log2(x+1))
mat_smiles <- apply(mat_smiles, 2, function(x) (x-mean(x))/sd(x))

d <- 1-cor(t(mat_smiles))
hc <- hclust(as.dist(d), "ward.D2")
sorted_smiles <- row.names(mat_smiles)[hc$order]

mat_smiles <- 
 mat_sub %>%
 filter(batch == "Cologne_A") %>%
group_by(SMILES, genotype) %>%
summarise(tot_int = sum(intensity)) %>%
spread(key = genotype, value = tot_int) %>%
mutate(pyk10 = pyk10/Col, cyp = cyp/Col) %>% 
select(-Col) %>%
data.frame(., row.names = .$SMILES)
...
p <- mat_stat %>%
# filter(batch == "Cologne_A") %>%
mutate(SMILES = factor(SMILES, levels = sorted_smiles)) %>%
ggplot(aes(y = genotype, x = SMILES, fill = scale(log2(tot_int+1)))) +
geom_point(shape = 21, size = 3, colour = c_black, alpha = 1) +
facet_grid(. ~ metabolite, space = "free", scale = "free", switch = "x") +
scale_fill_gradient2(midpoint = 0, high = c_green, mid = c_white, low = c_cudo_magenta) +
  ...
  labs(x = "", y = "", fill = "scales(log2(intensity+1))")

ggsave(p, file = paste0(figs, "heatmap_ra_nitrile_thiols_all.png"),
  units = "in",
  device = "png",
  width = 3,
  height = 2.5,
...
p <- mat_stat %>%
# filter(batch == "Cologne_A") %>%
cbind.data.frame(., mat_smiles[match(.$SMILES, row.names(mat_smiles)),c("pyk10", "cyp")]) %>%
mutate(SMILES = factor(SMILES, levels = sorted_smiles)) %>%
select(metabolite, SMILES, cyp, pyk10) %>%
gather(key = "genotype", value = "vsCol", -metabolite, -SMILES) %>%
mutate(genotype = factor(genotype, levels = c("pyk10", "cyp"))) %>%
ggplot(aes(y = genotype, x = SMILES, fill = log2(vsCol))) +
geom_point(shape = 21, size = 3, colour = c_black, alpha = 1) +
facet_grid(. ~ metabolite, space = "free", scale = "free", switch = "x") +
scale_fill_gradient2(midpoint = 0, high = c_green, mid = c_white, low = c_cudo_magenta) +
  theme_RTN +
  theme(
    ...
    panel.spacing = unit(0.0, "lines"), 
    strip.text.x = element_text(size = 2, angle = 90)) +
  labs(x = "", y = "", fill = "log2(vsCol)")

ggsave(p, file = paste0(figs, "heatmap_vsCol_nitrile_thiols_all.png"),
  units = "in",
  device = "png",
  width = 3,
  height = 2.5,
...
# Boxplot of the indole compounds
# parallel::mclapply(target, function(x){

#   idx <- str_detect(as.character(fdata$`Average.Mz`), x)
#   temp <- cbind.data.frame(mat[idx,], fdata[,1:5])
