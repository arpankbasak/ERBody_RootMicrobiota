...
load(paste0("./data/CAS_DE_family_RP.RData"))
...
# LFC matrix
lfc <- CAS_DE_family$logFC_P

# Subset of ASVs that are prevalant
temp.meta_RP <- temp.meta %>% filter(Compartment == "RP")
...
asv.prev <- asv.ra[idx,]
prev_taxa <- unique(all_exp_cut$taxa[match(row.names(asv.prev), all_exp_cut$taxa$asvf), "lineage_family"])

# Box-plot of the prev ASVs
df.prev <- as.data.frame(asv.prev) %>% 
mutate(tag = replace_na(all_exp_cut$taxa$lineage_family[match(row.names(.), all_exp_cut$taxa$asvf)], "Unclassified")) %>%
...
facet_grid(Soil_Batch ~ tag, scale = "free_x", switch = "both", space = "free_x") +
...
labs(x = "", y = "-log10(RA)")
ggsave(p, file = paste0(figs.out, "/RA_prevalant_RP_family_level.png"), 
            dpi = 600, 
            units = img, 
            device = "png", 
            bg = "transparent", 
...
geom_point(aes(colour = Genotype), size = 0.5, shape = 2, position = position_jitterdodge(1)) +
geom_boxplot(aes(colour = Genotype), fill = c_white, alpha = 0.7, size = 1, outlier.alpha = 0) +
facet_grid(Soil_Batch ~ tag, scale = "free", switch = "both", space = "free") +
scale_colour_manual(values = genotype$colours[which(genotype$short %in% as.character(df.prev$Genotype))]) +
theme_RTN +
theme(
  panel.spacing = unit(0.1, "lines"), 
  ...
  strip.text.y = element_text(size = 2)) +
labs(x = "", y = "-log10(RA)") +
ggsave(p, file = paste0(figs.out, "/RA_t20RP_family_level.png"), 
            dpi = 600, 
            units = img, 
            device = "png", 
            bg = "transparent", 
...
ggsave(p, file = paste0(figs.out, "/rank_abundance_top20_family_RP_family_level.png"), 
            dpi = 600, 
            units = img, 
            device = "png", 
            bg = "transparent", 
...
add_column(tag = row.names(.), .before = 1) %>%
mutate(tag = (all_exp_cut$taxa$lineage_family[match(row.names(.), all_exp_cut$taxa$asvf)])) %>%
gather(key = "SampleID", value = "RA", convert = FALSE, -tag) %>%
...
ggsave(p, filename = paste0(figs.out,"/prevelant_heatmap_family.png"),
       bg="transparent", 
       width=8, 
       height=5,
       units = img, 
...
theme(panel.spacing = unit(0.1, "lines"), 
      ...
      legend.text = element_text(size = 6, hjust = 0.5, vjust = 0.5)) +
labs(y="", x="", fill="", shape = "", size = "", colour = "")
ggsave(p, filename = paste0(figs.out,"/prevelant_heatmap_t20family_RP.png"),
       bg="transparent", 
       width=3.5, 
       height=6,
       units = img, 
...
summarise(RA = (sum(ra)), meanra = (mean(ra))) %>%
...
theme(panel.spacing = unit(0.1, "lines"), 
      ...
      legend.text = element_text(size = 6, hjust = 0.5, vjust = 0.5)) +
labs(y="", x="", fill="", shape = "", size = "", colour = "")
ggsave(p, filename = paste0(figs.out,"/barplot_t20family_RP.png"),
       bg="transparent", 
       width=3.5, 
       height=5,
       units = img, 
...
ggsave(p, filename = paste0(figs.out,"/correlogram_heatmap_family_RP.png"),
       bg="transparent", 
       width=3.5, 
       height= 3,
       units = img, 
...
for(i in c("nai1", "pyk10", "cyp", "myb")){
   ...
   for(j in c("nai1", "pyk10", "cyp", "myb")){
      ...
      if(j != i && dim(temp)[1] > 2){
        ...
        labs(x = i, y = j, fill = "")
        ggsave(p, file = paste0(figs.out, "/Parwise_RP_family_level_", j,"_", i,"_logFC.png"), 
                    dpi = 600, 
                    units = img, 
                    device = "png", 
                    bg = "transparent", 
        ...
        theme(panel.spacing = unit(0.5, "lines"), plot.title = element_text(size=4)) +
        labs(x = i, y = j, fill = "")
        ggsave(p, file = paste0(figs.out, "/Parwise_RP_family_level_", j,"_", i,"_logFC_tagged.png"), 
                    dpi = 600, 
                    units = img, 
                    device = "png", 
                    bg = "transparent", 
