...
load(paste0("./data/asvs_CAS_DE_asv.RData"))
...
temp.meta_E <- temp.meta %>% filter(Compartment == "E")
...
ggsave(p, filename = paste0(figs.out,"/correlogram_heatmap_asv_E.png"),
       bg="transparent", 
       width=3.5, 
       height= 3,
       units = img, 
...
for(i in c("nai1", "pyk10", "cyp", "myb")){
   ...
   for(j in c("nai1", "pyk10", "cyp", "myb")){
      ...
      if(j != i && dim(temp)[1] > 2){
        ...
        labs(x = i, y = j, fill = "")
        ggsave(p, file = paste0(figs.out, "/Parwise_E_", j,"_", i,"_logFC.png"), 
                    dpi = 600, 
                    units = img, 
                    device = "png", 
                    bg = "transparent", 
