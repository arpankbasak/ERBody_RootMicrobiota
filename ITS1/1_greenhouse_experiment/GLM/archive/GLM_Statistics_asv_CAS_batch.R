...
# Make factors
temp.meta <- temp.meta %>%
filter(!Compartment %in% c("rhizosphere"), soil == "CAS") %>%
mutate(
...
df <- temp.meta %>%
  ...
  mutate(exp_rep = paste(Experiment, Replicate, sep = "_"),
                      group = paste(genotype_short, compartment_short, Soil_Batch, sep = "_"))
...
# Grouping factors
levels <- c(sapply(compartment$short, paste, unique(paste(soilbatch$names, sep="_")), sep = "_"))
levels <- c(sapply(genotype$short, paste, unique(paste(levels, sep="_")), sep = "_"))
group <- factor(df$group, levels = levels[levels %in% unique(df$group)])
exp_rep <- droplevels(as.factor(df$exp_rep))
...
contrasts.mat <- limma::makeContrasts(
    ...
    compartment_Col_E_cas11 = (Col_E_CAS11 - soil_BS_CAS11),
    compartment_Col_RP_cas11 = (Col_RP_CAS11 - soil_BS_CAS11),

    compartment_nai1_E_cas11 = (nai1_E_CAS11 - soil_BS_CAS11),
    compartment_nai1_RP_cas11 = (nai1_RP_CAS11 - soil_BS_CAS11),

    compartment_pyk10_E_cas11 = (pyk10_E_CAS11 - soil_BS_CAS11),
    compartment_pyk10_RP_cas11 = (pyk10_RP_CAS11 - soil_BS_CAS11),

    compartment_cyp_E_cas11 = (cyp_E_CAS11 - soil_BS_CAS11),
    compartment_cyp_RP_cas11 = (cyp_RP_CAS11 - soil_BS_CAS11),

    compartment_myb_E_cas11 = (myb_E_CAS11 - soil_BS_CAS11),
    compartment_myb_RP_cas11 = (myb_RP_CAS11 - soil_BS_CAS11),

    genotype_nai1_E_cas11          = (  nai1_E_CAS11   - Col_E_CAS11 ),
    #nai2_E          = (  nai2_E   - Col_E ),
    genotype_pyk10_E_cas11         = ( pyk10_E_CAS11   - Col_E_CAS11 ),
    genotype_cyp_E_cas11           = (   cyp_E_CAS11   - Col_E_CAS11 ),
    genotype_myb_E_cas11           = (   myb_E_CAS11   - Col_E_CAS11 ),
    # bhlh_E = (   bhlh_E   - Col_E ),
    
    
    genotype_nai1_RP_cas11     = (  nai1_RP_CAS11 -   Col_RP_CAS11),
    # nai2_RP     = (  nai2_RP -   Col_RP),
    genotype_pyk10_RP_cas11    = ( pyk10_RP_CAS11 -   Col_RP_CAS11),
    genotype_cyp_RP_cas11      = (   cyp_RP_CAS11 -   Col_RP_CAS11  ),
    genotype_myb_RP_cas11      = (   myb_RP_CAS11 -    Col_RP_CAS11),
    # bhlh_RP = (   bhlh_RP   - Col_RP ),

    compartment_Col_E_cas13 = (Col_E_CAS13 - soil_BS_CAS13),
    compartment_Col_RP_cas13 = (Col_RP_CAS13 - soil_BS_CAS13),

    compartment_nai1_E_cas13 = (nai1_E_CAS13 - soil_BS_CAS13),
    compartment_nai1_RP_cas13 = (nai1_RP_CAS13 - soil_BS_CAS13),

    compartment_pyk10_E_cas13 = (pyk10_E_CAS13 - soil_BS_CAS13),
    compartment_pyk10_RP_cas13 = (pyk10_RP_CAS13 - soil_BS_CAS13),

    compartment_cyp_E_cas13 = (cyp_E_CAS13 - soil_BS_CAS13),
    compartment_cyp_RP_cas13 = (cyp_RP_CAS13 - soil_BS_CAS13),

    compartment_myb_E_cas13 = (myb_E_CAS13 - soil_BS_CAS13),
    compartment_myb_RP_cas13 = (myb_RP_CAS13 - soil_BS_CAS13),

    genotype_nai1_E_cas13          = (  nai1_E_CAS13   - Col_E_CAS13 ),
    #nai2_E          = (  nai2_E   - Col_E ),
    genotype_pyk10_E_cas13         = ( pyk10_E_CAS13   - Col_E_CAS13 ),
    genotype_cyp_E_cas13           = (   cyp_E_CAS13   - Col_E_CAS13 ),
    genotype_myb_E_cas13           = (   myb_E_CAS13   - Col_E_CAS13 ),
    # bhlh_E = (   bhlh_E   - Col_E ),
    
    
    genotype_nai1_RP_cas13     = (  nai1_RP_CAS13 -   Col_RP_CAS11),
    # nai2_RP     = (  nai2_RP -   Col_RP),
    genotype_pyk10_RP_cas13    = ( pyk10_RP_CAS13 -   Col_RP_CAS13),
    genotype_cyp_RP_cas13      = (   cyp_RP_CAS13 -   Col_RP_CAS13  ),
    genotype_myb_RP_cas13      = (   myb_RP_CAS13 -    Col_RP_CAS13),
...
write.table(logFC_P, paste0(stats.out, "/stats_logFC.P_combat_batch.txt"), sep="\t", 
...
write.table(count.mat, paste0(stats.out, "/stats_number_of_enriched_depleted_combat_batch.txt"), 
...
write.table(DE, paste0(stats.out, "/stats_enrichment_test.significance_table_combat_batch.txt"), sep="\t", 
            quote = T, row.names = T, col.names = NA)

log2cpm <- cpm(de, prior.count = 2, log = T)

write.table(log2cpm, paste0(stats.out, "/stats_log2cpm_combat_batch.txt"), sep="\t", 
            quote = F, col.names = NA, row.names = T)


# Make object an RObject as output
CAS_DE_asv_batch <- list(
  logFC_P = logFC_P, 
  DE_table = count.mat,
  cpm = log2cpm
)

save(list = "CAS_DE_asv_batch", file = paste0("./data/asvs_CAS_DE_asv_batch.RData"))
