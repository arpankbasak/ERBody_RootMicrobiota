...
write.table(logFC_P, paste0(stats.out, "/stats_logFC.P_RP.txt"), sep="\t", 
...
write.table(count.mat, paste0(stats.out, "/stats_number_of_enriched_depleted_combat_RP.txt"), 
...
write.table(DE, paste0(stats.out, "/stats_enrichment_test.significance_table_combat_RP.txt"), sep="\t", 
            quote = T, row.names = T, col.names = NA)

log2cpm <- cpm(de, prior.count = 2, log = T)

write.table(log2cpm, paste0(stats.out, "/stats_log2cpm_combat_RP.txt"), sep="\t", 
            quote = F, col.names = NA, row.names = T)
...
save(list = "CAS_DE_asv", file = paste0("./data/asvs_CAS_DE_asv_RP.RData"))
