...
temp.meta <- rbind.data.frame(meta$datasets[["CAS"]]) %>% filter(Experiment != "pro3")
...
# Make factors
temp.meta <- temp.meta %>%
filter(!Compartment %in% c("rhizosphere", "endosphere"), soil == "CAS") %>%
...
levels <- c(sapply(levels, paste, unique(paste(soilbatch$names, sep="_")), sep = "_"))
...
idx <- colnames(model) == "exp_reppro20_5"
model <- model[, !idx]
...
#     compartment_Col_RP_cas = (Col_E - soil_BS),
#     # compartment_Col_RP = (Col_RP - soil_BS),

#     compartment_nai1_RP_cas = (nai1_E - soil_BS),
#     # compartment_nai1_RP = (nai1_RP - soil_BS),

#     compartment_pyk10_RP_cas = (pyk10_E - soil_BS),
#     # compartment_pyk10_RP = (pyk10_RP - soil_BS),

#     compartment_cyp_RP_cas = (cyp_E - soil_BS),
#     # compartment_cyp_RP = (cyp_RP - soil_BS),

#     compartment_myb_RP_cas = (myb_E - soil_BS),
#     # compartment_myb_RP = (myb_RP - soil_BS),

#     genotype_nai1_RP_cas          = (  nai1_E   - Col_E ),
#     #nai2_E          = (  nai2_E   - Col_E ),
#     genotype_pyk10_RP_cas         = ( pyk10_E   - Col_E ),
#     genotype_cyp_RP_cas           = (   cyp_E   - Col_E ),
#     genotype_myb_RP_cas           = (   myb_E   - Col_E ),
#     # bhlh_E = (   bhlh_E   - Col_E ),
    
    
#     # genotype_nai1_RP     = (  nai1_RP -   Col_RP),
#     # # nai2_RP     = (  nai2_RP -   Col_RP),
#     # genotype_pyk10_RP    = ( pyk10_RP -   Col_RP),
#     # genotype_cyp_RP      = (   cyp_RP -   Col_RP  ),
#     # genotype_myb_RP      = (   myb_RP -    Col_RP),
...
contrasts.mat <- limma::makeContrasts(
genotype_nai1_RP_CAS13     = (  nai1_RP_CAS13 - Col_RP_CAS13 ),
...
write.table(logFC_P, paste0(stats.out, "/stats_logFC.P_family_RP.txt"), sep="\t", 
...
write.table(count.mat, paste0(stats.out, "/stats_number_of_enriched_depleted_family_RP.txt"), 
...
write.table(DE, paste0(stats.out, "/stats_enrichment_test.significance_table_family_RP.txt"), sep="\t", 
            quote = T, row.names = T, col.names = NA)

log2cpm <- cpm(de, prior.count = 2, log = T)

write.table(log2cpm, paste0(stats.out, "/stats_log2cpm_family_RP.txt"), sep="\t", 
            quote = F, col.names = NA, row.names = T)
...
save(list = "CAS_DE_family", file = paste0("./data/CAS_DE_family_RP.RData"))
