...
ra_RPcut_11 <- asv.ra[, idx]
ra_RPcut_13 <- asv.ra[, !idx]
​
# Compute prevelance
idx <- (rowSums(ra_RPcut_11 > 0) > ncol(ra_RPcut_11)/2) & (rowSums(ra_RPcut_13 > 0) > ncol(ra_RPcut_13)/2)
asv.prev <- asv.ra[idx,]
​
idx <- (rownames(lfc) %in% rownames(asv.prev))
lfc.prev <- lfc[idx, ]

ra.prev <- as.data.frame(asv.prev) %>% 
mutate(tag = replace_na(all_exp_cut$taxa$Family[match(row.names(.), all_exp_cut$taxa$asvf)], "Unclassified")) %>%
...
ggsave(p, file = paste0(figs.out, "/rank_abundance_top20_family_RP.png"), 
            dpi = 600, 
            units = img, 
            device = "png", 
            bg = "transparent", 
...
mutate(
	...
	Soil_Batch = factor(Soil_Batch, levels = soilbatch$names[which(soilbatch$names %in% .$Soil_Batch)]),
	sig = PValue <= 0.05,
	higher_tag = replace_na(all_exp_cut$taxa$Family[match(.$tag, all_exp_cut$taxa$asvf)], "Unclassified")
) %>%
...
ggsave(p, filename = paste0(figs.out,"/prevelant_heatmap_asvs_RP.png"),
       bg="transparent", 
       width=8, 
       height=5,
       units = img, 
...
ggsave(p, filename = paste0(figs.out,"/correlogram_heatmap_asv_RP.png"),
       bg="transparent", 
       width=3.5, 
       height= 3,
       units = img, 
...
for(i in c("nai1", "pyk10", "cyp", "myb")){
   ...
   for(j in c("nai1", "pyk10", "cyp", "myb")){
      ...
      if(j != i && dim(temp)[1] > 2){
        ...
        labs(x = i, y = j, fill = "")
        ggsave(p, filename = paste0(figs.out, "/Parwise_RP_", j,"_", i,"_logFC.png"), 
                    dpi = 600, 
                    units = img, 
                    device = "png", 
                    bg = "transparent", 
