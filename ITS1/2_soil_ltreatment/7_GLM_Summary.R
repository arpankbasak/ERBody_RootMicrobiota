...
cluster_taxa_reps <- asv_clusterd$lfc_annotated.ra %>%
...
scale_colour_manual(values = c(`TRUE` = c_black, `FALSE` = NA)) +
    ...
    theme(panel.spacing = unit(0.1, "lines"), panel.border = element_rect(fill="transparent", colour=NA),
          ...
          legend.text = element_text(size = 2, hjust = 0.5, vjust = 0.5)) +
    labs(y="", x="", fill="")) +
    ggsave(filename = paste0(figs.out,"/UNITE_annotated_lfc_sorted_sig.png"),
           bg="transparent", 
           width=2, 
           height=8,
           units = img, 
...
theme() +
labs(y="", x="", fill="")
ggsave(sct_plot, filename = paste0(figs.out,"/pyk10_vs_cyp_asv_.png"),
           bg="transparent", 
           width=5, 
           height=5,
           units = img, 
