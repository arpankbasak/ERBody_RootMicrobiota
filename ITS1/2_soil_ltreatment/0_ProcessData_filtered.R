...
asv_tab <- read.table(paste0(asv.itsf, "ASV_table.txt"), sep = "\t", header = T, stringsAsFactors = F, row.names = 1)
asv_lib <- read.table(paste0(asv.itsf, "ASV_lib_stat.txt"), sep = "\t", header = T, stringsAsFactors = F, row.names = 1)
...
spike <- asv_tab[spike_itsf[,2],]
stats_spike <- as.data.frame(t(spike))
spike_detects <- row.names(stats_spike)[stats_spike[,1] != 0]
spike_id <- row.names(spike)

data.frame(total_asvs = colSums(asv_tab[row.names(asv_tab) %in% spike_id,]), SampleIDs = as.factor(names(colSums(asv_tab)))) %>%
filter(total_asvs > 0) %>%
...
ggsave("./figures/diagnosis/ASVS_across_samples_spike_in.png",
  units = img,
  width = 3,
  height = 7,
  bg = "white")
...
# Metadata 
taxa_s <- read.table(paste0(asv.itsf, "ASV_taxonomy_silva.txt"), sep = "\t", stringsAsFactors = F, fill = T, header = T)
taxa_gg <- read.table(paste0(asv.itsf, "ASV_taxonomy_gg.txt"), sep = "\t", stringsAsFactors = F, fill = T, header = T)
taxa_rdp <- read.table(paste0(asv.itsf, "ASV_taxonomy_rdp.txt"), sep = "\t", stringsAsFactors = F, fill = T, header = T)
taxa_syncom <- read.table(paste0(asv.itsf, "ASV_robust_syncom.txt"), sep = "\t", stringsAsFactors = F, fill = T, header = T)
map_syncom <- read.table(map_itsf_cc, sep = "\t", stringsAsFactors = F, fill = T, header = T)
#map <- read.table(paste0(asv.itsf, "asvf_map.txt"), sep = "\t", stringsAsFactors = F, fill = T, header = F)
...
data.frame(total_asvs = colSums(asv_tab), SampleIDs = as.factor(names(colSums(asv_tab)))) %>%
...
theme_RTN +
theme(axis.text.y = element_text(size = 2)) +
ggsave("./figures/diagnosis/ASVS_across_samples.png",
  units = img,
  width = 3,
  height = 7,
  bg = "white")
...
parallel::mclapply(colnames(asv_tab), function(x){
  ...
  qplot(log10(asv_tab[,x]+1), main = x, geom = "histogram", bins = 100) +
  ggsave(paste0("./figures/diagnosis/ASV_histogram_", x,".png"),
  units = img,
  width = 4,
  height = 2,
  bg = "white")
...
#===== Taxonomy table =====
taxa <- taxa %>% add_column(asvf = row.names(taxa), .before = 1) %>%
  filter(!Kingdom %in% c("Eukaryota", "Archaea"))
row.names(taxa) <- taxa$asvf

taxa_cut <- taxa[row.names(taxa) %in% row.names(asv_cut),]
asv_sorted <- taxa %>% group_by(Kingdom, Phylum, Class, Order, Family, asvf) %>% 
                            summarise(lineage_asvf = 
                                      paste(Kingdom, Phylum, Class, Order, Family, asvf, 
                                            sep = ";")) %>% arrange(lineage_asvf) %>%
                            data.frame(.)

asv_sorted_cut <- asv_sorted[asv_sorted$asvf %in% row.names(asv_cut),]
...
family_sorted_cut <- family_sorted[family_sorted$asvf %in% row.names(asv_cut),]

taxa$lineage_asvf <- asv_sorted$lineage_asv[match(taxa$asvf, asv_sorted$asvf)]
taxa$lineage_family <- family_sorted$lineage_family[match(taxa$asvf, family_sorted$asvf)]
taxa <- taxa %>% mutate(tag_family = paste(Class, Order, Family, asvf, sep = ";"))

taxa_cut$lineage_asvf <- asv_sorted_cut$lineage_asv[match(taxa_cut$asvf, asv_sorted_cut$asvf)]
taxa_cut$lineage_family <- family_sorted_cut$lineage_family[match(taxa_cut$asvf, family_sorted_cut$asvf)]

taxa_cut <- taxa_cut %>% mutate(tag_family = paste(Class, Order, Family, asvf, sep = ";"))
...
asv_sorted_cut <- asv_sorted[asv_sorted$asvf %in% row.names(aa),]
...
family_sorted_cut <- family_sorted[family_sorted$asvf %in% row.names(aa),]

taxa$lineage_asvf <- asv_sorted$lineage_asv[match(taxa$asvf, asv_sorted$asvf)]
taxa$lineage_family <- family_sorted$lineage_family[match(taxa$asvf, family_sorted$asvf)]
taxa <- taxa %>% mutate(tag_family = paste(Class, Order, Family, asvf, sep = ";"))

taxa_cut_aa$lineage_asvf <- asv_sorted_cut$lineage_asv[match(taxa_cut_aa$asvf, asv_sorted_cut$asvf)]
taxa_cut_aa$lineage_family <- family_sorted_cut$lineage_family[match(taxa_cut_aa$asvf, family_sorted_cut$asvf)]

taxa_cut_aa <- taxa_cut_aa %>% mutate(tag_family = paste(Class, Order, Family, asvf, sep = ";"))
...
select(asvf, Species) %>%
mutate(taxa_db = taxa_cut$lineage_asvf[match(.$asvf, taxa_cut$asvf)],
  taxa_db_lineage = taxa_cut$lineage[match(.$asvf, taxa_cut$asvf)]
  ) %>%
cbind.data.frame(., map_syncom[match(.$Species, map_syncom$ID), c("kingdom","phylum","class","order", "family")]) %>%
mutate(taxa_atsphere = paste(kingdom, phylum, class, order, family, asvf, sep = ";"),
  taxa_atsphere_lineage = paste(kingdom, phylum, class, order, family, sep = ";"),
  taxa_atsphere_strain = paste(kingdom, phylum, class, order, family, Species, asvf, sep = ";"),
  taxa_atsphere_strain_lineage = paste(kingdom, phylum, class, order, family, Species, sep = ";")
)
row.names(taxa_atcc) <- taxa_atcc$asvf
...
# Unspiked
asv_taxa <- as.data.frame(asv_cut) %>% add_column(asvf = row.names(.), .before = 1)
asv_taxa <- melt(asv_taxa)
colnames(asv_taxa) <- c("asvf", "SampleID", "counts")
asv_taxa$lineage_family <- taxa_cut$lineage_family[match(asv_taxa$asvf, taxa_cut$asvf)]
...
ra_tab_taxa_cut <- as.data.frame(ra_taxa_cut) %>% add_column(asvf = row.names(asv_taxa_cut), .before = 1) %>% data.frame(.)
ra_tab_taxa_cut <- melt(ra_tab_taxa_cut)
colnames(ra_tab_taxa_cut) <- c("asvf", "SampleID", "RA")
...
# Absolute
asv_taxa <- as.data.frame(aa) %>% add_column(asvf = row.names(.), .before = 1)
asv_taxa <- melt(asv_taxa)
colnames(asv_taxa) <- c("asvf", "SampleID", "counts")
asv_taxa$lineage_family <- taxa_cut_aa$lineage_family[match(asv_taxa$asvf, taxa_cut_aa$asvf)]
...
aa_tab_taxa_cut <- as.data.frame(aa_taxa_cut) %>% add_column(asvf = row.names(asv_taxa_cut), .before = 1) %>% data.frame(.)
aa_tab_taxa_cut <- melt(aa_tab_taxa_cut)
colnames(aa_tab_taxa_cut) <- c("asvf", "SampleID", "AA")
...
# Syncom
asv_taxa <- as.data.frame(ra_cut_sync) %>% add_column(asvf = row.names(.), .before = 1)
asv_taxa <- melt(asv_taxa)
colnames(asv_taxa) <- c("asvf", "SampleID", "counts")
asv_taxa$lineage_family <- taxa_atcc$taxa_atsphere_strain_lineage[match(asv_taxa$asvf, taxa_atcc$asvf)]
asv_taxa <- asv_taxa %>% group_by(lineage_family, SampleID) %>% 
summarise(counts = sum(counts)) %>% 
spread(key = "SampleID", value = "counts", convert = FALSE) %>% 
data.frame(., stringsAsFactors = F)
...
add_column(asvf = row.names(asv_taxa_cut), .before = 1) %>% data.frame(.)
ra_tab_taxa_cut_syn <- melt(ra_tab_taxa_cut_syn)
colnames(ra_tab_taxa_cut_syn) <- c("asvf", "SampleID", "RA")
...
# Syncom -- Absolute
asv_taxa <- as.data.frame(aa_sync) %>% add_column(asvf = row.names(.), .before = 1)
asv_taxa <- melt(asv_taxa)
colnames(asv_taxa) <- c("asvf", "SampleID", "counts")
asv_taxa$lineage_family <- taxa_atcc_spike$taxa_atsphere_strain_lineage[match(asv_taxa$asvf, taxa_atcc_spike$asvf)]
asv_taxa <- asv_taxa %>% group_by(lineage_family, SampleID) %>% 
summarise(counts = sum(counts)) %>% 
spread(key = "SampleID", value = "counts", convert = FALSE) %>% 
data.frame(., stringsAsFactors = F)
...
colnames(aa_tab_taxa_cut_syn) <- c("asvf", "SampleID", "AA")
...
save(list = c("clusters_unspiked", "clusters_spiked"), 
  file = paste0("./data/asv_clusters_itsf.Rdata"))
