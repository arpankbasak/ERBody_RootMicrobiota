...
idx <- unique(c(row.names(ST_DE_asv_Extract$logFC_P), row.names(ST_DE_asv_Exudate$logFC_P)))
lfc <- data.frame(tag = idx) %>% cbind.data.frame(., 
  ST_DE_asv_Extract$logFC_P[.$tag,], 
  ST_DE_asv_Exudate$logFC_P[.$tag,]
)
...
asv.ra <- apply(temp.mat[ix,], 2, function(x) x/sum(x))
...
lfc_annotated <- cbind.data.frame(lfc, 
  lineage_family = all_exp_cut$taxa$lineage_family[match(row.names(lfc), all_exp_cut$taxa$asvf)])
...
save(list = "asv_clusterd", file = "./data/GLM_clustered_asv.RData")
